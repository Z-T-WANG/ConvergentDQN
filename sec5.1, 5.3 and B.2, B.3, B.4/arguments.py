...
def get_args():
    ...
    parser.add_argument('--algorithm', type=str, default="CDQN")
    # Basic Arguments
    ...
    parser.add_argument('--gamma', type=float, default=-1., metavar='γ',
                        help='Discount factor')
    reward_shaping = parser.add_mutually_exclusive_group()
    reward_shaping.add_argument('--no-clip', action='store_true',
                        help='Use the raw reward instead of clipping the reward to {-1, 0, +1}')
    reward_shaping.add_argument('--transform-Q', action='store_true',
                        help='Learn the transformed Q function, i.e. sgn(Q)*(sqrt(abs(Q)+1)-1) + 0.001*Q')
    ...
    parser.add_argument('--auto-init', action='store_true',
                        help='Use heuristics to automatically set \gamma and reward scale, and initialize the bias of the network final layer to be the mean')
    ...
    parser.add_argument('--evaluate', action='store_true',
                        help='Evaluate only')
    parser.add_argument('--num-trial', type=int, default=400, help = "The number of test episodes to evaluate the test performance")
    ...
    parser.add_argument('--eval-value-error', action='store_true',
                        help='To evaluate the deviation of the predicted Q from the true value function')
    ...
    if args.gamma == -1.:
        if args.auto_init: args.gamma = None
        else: args.gamma = 0.99
