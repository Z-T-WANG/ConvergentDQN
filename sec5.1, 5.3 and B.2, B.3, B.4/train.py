...
from model import DQN, inverse_transform_Q, transform_Q, auto_initialize, transform_backpropagate, compute_value_error
from common.replay_buffer import ReplayBuffer, PrioritizedReplayBuffer
from common.save_agent import Agent_History
...
def train(env, args):
    ...
    if args.eval_value_error:
        reward_record = [[] for _i in range(num_task)]
        value_record = [[] for _i in range(num_task)]
    ...
    auto_inited = False
    if args.save_best:
        training_performance_comparer = Agent_History(args, n_best = 5, num_avg_episode = 40)
        last_episode_step = [0 for _i in range(num_task)] 
        ...
        for _i, (env, state, action, Qs, bestAction, reward) in enumerate(zip(envs, states, actions, Qss, bestActions, rewards)): 
            ...
            if clip_reward:
                ...
                if reward_scale is None: 
                    if reward != 0.: 
                        reward_scale = max(reward/(15/2.), 1.); reward /= reward_scale 
            ...
            life_lengths[_i] += 1 
            if args.eval_value_error: 
                reward_record[_i].append(reward); value_record[_i].append(Qs[action])
            ...
            if done:
                ...
                if env.unwrapped.ale.game_over() or "TimeLimit.truncated" in info:
                    ...
                    if not args.silent:
                        ...
                        if args.save_best:
                            if step_idx > args.learning_start:
                                training_performance_comparer.add_training_performance(episode_rewards[_i], (step_idx+last_episode_step[_i]+num_task)/2.)
                            last_episode_step[_i] = step_idx
                    episode_rewards[_i] = 0.
                states[_i] = env.reset()
                if args.eval_value_error:
                    if not os.path.isdir(args.env): os.mkdir(args.env)
                    deviation, squared_error = compute_value_error(reward_record[_i], value_record[_i], args.gamma)
                    reward_record[_i].clear(); value_record[_i].clear()
                    with open(os.path.join(args.env, '{}_{}ValueErr.txt'.format(currentTask, args.comment)), 'a') as f:
                        f.write('{:.0f}\t{}\t{}\n'.format(step_idx*4, deviation, squared_error))
            else:
                states[_i] = next_state

            # cache the agent for saving 
            if step_idx % 10000 == 0 and args.save_best and step_idx > args.learning_start and not args.silent:
                optimizer.zero_grad(set_to_none=True) 
                training_performance_comparer.add_agent(copy.deepcopy(current_model.state_dict()), step_idx) 
...
def compute_td_loss(current_model, target_model, replay_buffer, optimizer, args, beta=None):
    ...
    """
    global i_count, accu1, accu2, accu_loss, accu_CDQNratio
    ...
    if i_count % report_period == 0 and accu1 != 0.: 
        ...
        if not args.silent:
            ...
            if args.currentTask == "CDQN":
                with open(os.path.join(args.env, '{}loss_ratio_{}.txt'.format(args.currentTask, args.comment)), 'a') as f:
                    f.write('{:.0f}\t{}\n'.format((i_count*args.train_freq+args.learning_start)*4, accu_CDQNratio/report_period))
        accu_loss = 0.; accu_CDQNratio = 0.
